echo -e "\n${BLUE}:package: Checking Node.js installation...${NC}"
echo -e "${GREEN}:white_check_mark: Node.js $NODE_VERSION is installed${NC}"
echo -e "${GREEN}:white_check_mark: npm $NPM_VERSION is installed${NC}"
echo -e "\n${BLUE}:package: Installing server dependencies...${NC}"
echo -e "\n${BLUE}:package: Installing client dependencies...${NC}"
echo -e "\n${BLUE}:wrench: Setting up environment files...${NC}"
echo -e "\n${BLUE}:file_cabinet:  Checking for Prisma setup...${NC}"
echo -e "\n${BLUE}:file_folder: Creating necessary directories...${NC}"
echo -e "${GREEN}:white_check_mark: Directories created${NC}"
echo -e "\n=========================================="
#!/usr/bin/env bash
...
echo ":rocket: ShopSmart Idempotent Setup"
echo "================================"
...
check_command() {
    if ! command -v "$1" &> /dev/null; then
        echo -e "${RED}:x: $1 is not installed. Please install it and re-run this script.${NC}"
        return 1
    fi
    return 0
}
...
echo -e "${GREEN}:white_check_mark: Node and npm are available${NC}"
...
INSTALL_IF_NEEDED() {
    ...
    local P="$1"
    if [ ! -d "$P" ]; then
        echo -e "${RED}:x: Directory $P not found${NC}"
        return 1
    fi
    if [ "$FORCE" = true ] || [ ! -d "$P/node_modules" ]; then
        ...
        popd > /dev/null
        echo -e "${GREEN}:white_check_mark: Installed $P dependencies${NC}"
    else
        echo -e "${GREEN}:white_check_mark: Skipping $P install (node_modules exists). Use --force to reinstall.${NC}"
    fi
}
...
EOF
    echo -e "${GREEN}:white_check_mark: Created server/.env${NC}"
else
    echo -e "${GREEN}:white_check_mark: server/.env already exists${NC}"
fi
...
EOF
    echo -e "${GREEN}:white_check_mark: Created client/.env${NC}"
else
    echo -e "${GREEN}:white_check_mark: client/.env already exists${NC}"
fi

# Create useful directories (idempotent)
echo -e "\n${BLUE}Creating directories...${NC}"
mkdir -p server/logs server/uploads
echo -e "${GREEN}:white_check_mark: Directories ensured${NC}"
...
if [ -f server/prisma/schema.prisma ]; then
    ...
    if (cd server && (npx --no-install prisma -v) > /dev/null 2>&1) || [ -x "server/node_modules/.bin/prisma" ]; then
        ...
        popd > /dev/null
        echo -e "${GREEN}:white_check_mark: Prisma steps completed (if applicable)${NC}"
    else
...
echo -e "\n=========================================="
echo -e "${GREEN}:white_check_mark: Setup finished (idempotent).${NC}"
echo -e "=========================================="
echo -e "\nNext steps:"
